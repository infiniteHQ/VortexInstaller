...
void InstallAppWindow::RenderInstallVortex() {
  {
    ...
    CherryKit::ImageLocalCentered(
        Cherry::GetPath("resources/imgs/vx_banner.png"), x, y);
  ...
  CherryNextComponent.SetProperty(
      "size_x", std::to_string(CherryGUI::GetContentRegionAvail().x - 6.0f));
...
void InstallAppWindow::RenderLicenseAgreement() {
  static std::string file_content =
      ReadFile(Cherry::GetPath("resources/license/apache2.txt"));
  ...
  CherryGUI::BeginChild("TextZone", ImVec2(0, available_height), false,
  ...
  std::string text =
      CherryApp.GetLocale("loc.accept") + CherryApp.GetLocale("loc.close");
  ImVec2 to_remove = CherryGUI::CalcTextSize(text.c_str());
  CherryGUI::SetCursorPosX(CherryGUI::GetContentRegionMax().x - to_remove.x -
                           50);
  CherryGUI::SetCursorPosY(CherryGUI::GetContentRegionMax().y - 35.0f);
  ...
  Cherry::SetNextComponentProperty("color_bg_hovered", "#C3FF53FF");
  Cherry::SetNextComponentProperty("color_text", "#121212FF");
  if (CherryKit::ButtonText(CherryApp.GetLocale("loc.accept"))
          .GetData("isClicked") == "true") {
...
void InstallAppWindow::RenderSummary() {
  ...
  float button_width =
      (available_size.x - CherryGUI::GetStyle().ItemSpacing.x) / 2 - 42.0f;

  if (!m_Data->g_Request) {
    m_Data->g_UseNet = false;
    CherryGUI::TextColored(
        Cherry::HexToRGBA("#555555FF"),
        Cherry::GetLocale("loc.install.offline_mode").c_str());
  }
  if (!m_Data->g_NetFetched && m_Data->g_Request) {
    m_Data->g_UseNet = false;
    CherryGUI::TextColored(Cherry::HexToRGBA("#555555FF"),
                           Cherry::GetLocale("loc.install.wait_fetch").c_str());
  }

  if (!m_Data->m_BuiltinLauncherExist && !m_Data->g_Request) {
    m_Data->g_UseNet = true;
    CherryGUI::TextColored(Cherry::HexToRGBA("#FF3535FF"),
                           Cherry::GetLocale("loc.install.no_builtin").c_str());
  }
  ...
  if (m_Data->g_NetFetched) {
    ...
    if (CherryGUI::ImageSizeButtonWithText(
            ...
            button_width, label.c_str(), ImVec2(-FLT_MIN, 0.0f), ImVec2(0, 0),
...

  {
    ...
    if (m_Data->m_BuiltinLauncherExist) {
      label = Cherry::GetLocale("loc.install.builtin_install") + "\n(Latest " +
              m_Data->m_BuiltinLauncher.platform + " w/ " +
              m_Data->m_BuiltinLauncher.arch + " v. " +
              m_Data->m_BuiltinLauncher.version + ")";
    }
  ...
  if (ShowPackagesSelection) {
        ...
        [=]() {}, 155.0f, 85.0f,
        {
            [=]() {
              CherryKit::ImageLocal(
                  Cherry::GetPath("resources/imgs/package_base.png"), 155, 50);
            },
            [=]() {
    ...
    blocks.push_back(CherryKit::BlockVerticalCustom(
        [=]() { package_one = !package_one; }, 155.0f, 85.0f,
        {
            [=]() {
              CherryKit::ImageLocal(
                  Cherry::GetPath("resources/imgs/package_sys.png"), 155, 50);
            },
            [=]() {
              CherryStyle::AddMarginX(5.0f);
              CherryStyle::RemoveMarginY(5.0f);
    ...
    blocks.push_back(CherryKit::BlockVerticalCustom(
        [=]() {}, 155.0f, 85.0f,
        {
            [=]() {
              CherryKit::ImageLocal(
                  Cherry::GetPath("resources/imgs/package_tools.png"), 155, 50);
            },
            [=]() {
              CherryStyle::AddMarginX(5.0f);
              CherryStyle::RemoveMarginY(5.0f);
    ...
    blocks.push_back(CherryKit::BlockVerticalCustom(
        [=]() {}, 155.0f, 85.0f,
        {
            [=]() {
              CherryKit::ImageLocal(
                  Cherry::GetPath("resources/imgs/package_em.png"), 155, 50);
            },
            [=]() {
              CherryStyle::AddMarginX(5.0f);
              CherryStyle::RemoveMarginY(5.0f);
  ...
  if (m_Data->m_FolderAlreadyExist) {
    std::string label_install =
        Cherry::GetLocale("loc.install.install_path_is") + " : " +
    ...
    CherryGUI::TextColored(
        ...
        Cherry::GetLocale("loc.install.install_path_already_exist").c_str());
    CherryGUI::Checkbox(
        Cherry::GetLocale("loc.install.erase_and_install").c_str(),
        &CanInstall);
    CherryGUI::Separator();
  }
  std::string text = CherryApp.GetLocale("loc.confirg_install") +
                     CherryApp.GetLocale("loc.close");
  ImVec2 to_remove = CherryGUI::CalcTextSize(text.c_str());
  CherryGUI::SetCursorPosX(CherryGUI::GetContentRegionMax().x - to_remove.x -
                           50);
  CherryGUI::SetCursorPosY(CherryGUI::GetContentRegionMax().y - 35.0f);
...

  {
    ...
    if (CherryKit::ButtonText(CherryApp.GetLocale("loc.confirg_install"))
            .GetData("isClicked") == "true") {
      m_SelectedChildName = "?loc:loc.child.installation";
...
void InstallAppWindow::RenderInstall() {
  ...
  if (m_Data->result == "processing") {
    ...
    std::string text = CherryApp.GetLocale("loc.launch_launcher") +
                       CherryApp.GetLocale("loc.close");
    ImVec2 to_remove = CherryGUI::CalcTextSize(text.c_str());
    CherryGUI::SetCursorPosX(CherryGUI::GetContentRegionMax().x - to_remove.x -
                             50);
...
void InstallAppWindow::CheckExistingInstallation(
...
InstallAppWindow::InstallAppWindow(
  ...
  this->AddChild("?loc:loc.child.license",
...
void InstallAppWindow::AddChild(const std::string &child_name,
...
void InstallAppWindow::SetChildState(const std::string &child_name,
                                     const bool &state) {
  auto it = m_Childs.find(child_name);
  if (it != m_Childs.end()) {
...
std::shared_ptr<InstallAppWindow>
InstallAppWindow::Create(const std::string &name,
                         const std::shared_ptr<VortexInstallerData> &data) {
  auto instance =
      std::shared_ptr<InstallAppWindow>(new InstallAppWindow(name, data));
...
std::function<void()>
InstallAppWindow::GetChild(const std::string &child_name) {
  auto it = m_Childs.find(child_name);
  if (it != m_Childs.end()) {
