...
namespace VortexInstaller {
  ...
  void UpdateAppWindow::RenderUpdateVortex() {
    ...
    if (!m_Data->g_Request) {
      CherryKit::TextWrapped(CherryApp.GetLocale("loc.update.explain_no_internet"));
    } else {
      if (!m_Data->g_VortexLauncherOutdated) {
        CherryKit::TextWrapped(CherryApp.GetLocale("loc.update.explain_is_updated"));
      } else {
        CherryKit::TextWrapped(CherryApp.GetLocale("loc.update.explain_can_update"));
      }
    }
    ...
    std::string text = CherryApp.GetLocale("loc.update") + CherryApp.GetLocale("loc.close");
  ...
  */
  ...
  void UpdateAppWindow::RenderUpdate() {
    ...
    if (m_Data->finished) {
      ...
      Cherry::SetNextComponentProperty("color_bg_hovered", "#C3FF53FF");
      Cherry::SetNextComponentProperty("color_text", "#121212FF");
      if (CherryKit::ButtonText(Cherry::GetLocale("loc.finish")).GetData("isClicked") == "true") {
        Cherry::Application().Get().Close();
      }
    }
  }
  ...
  UpdateAppWindow::UpdateAppWindow(const std::string &name, const std::shared_ptr<VortexInstallerData> &data)
    ...
    this->AddChild("?loc:loc.child.update_vortex", [this]() { RenderUpdateVortex(); });
