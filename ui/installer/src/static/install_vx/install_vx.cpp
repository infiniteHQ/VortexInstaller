...
void VortexInstallAppWindow::RenderInstallVortex() {
  {
    ...
    CherryKit::ImageLocalCentered(
        Cherry::GetPath("resources/imgs/update_banner.png"), x, y);
  ...
  CherryKit::SeparatorText(
      Cherry::GetLocale("loc.vxinstall.available_version") + " :");
        ...
        [=]() {}, 155.0f, 85.0f,
        {
            [=]() {
              CherryKit::ImageLocal(
                  Cherry::GetHttpPath(m_Data->m_SelectedVortexVersion.banner),
                  155, 50);
            },
            [=]() {
  ...
  CherryGUI::TextWrapped(
      Cherry::GetLocale("loc.vxinstall.choose_target_folder").c_str(),
      "#787878FF");

  CherryNextComponent.SetProperty("size_x",
                                  CherryGUI::GetContentRegionAvail().x);
  int selected = std::stoi(CherryKit::ComboText("", &m_Data->m_VortexPools, 0)
...
VortexInstallAppWindow::VortexInstallAppWindow(
  ...
  this->AddChild("?loc:loc.child.installation_vortex_editor",
                 [this]() { RenderInstallVortex(); });

  std::shared_ptr<Cherry::AppWindow> win = m_AppWindow;
}

void VortexInstallAppWindow::AddChild(const std::string &child_name,
...
void VortexInstallAppWindow::SetChildState(const std::string &child_name,
                                           const bool &state) {
  auto it = m_Childs.find(child_name);
  if (it != m_Childs.end()) {
...
std::shared_ptr<VortexInstallAppWindow> VortexInstallAppWindow::Create(
    const std::string &name, const std::shared_ptr<VortexInstallerData> &data) {
  auto instance = std::shared_ptr<VortexInstallAppWindow>(
      new VortexInstallAppWindow(name, data));
...
std::function<void()>
VortexInstallAppWindow::GetChild(const std::string &child_name) {
  auto it = m_Childs.find(child_name);
  if (it != m_Childs.end()) {
